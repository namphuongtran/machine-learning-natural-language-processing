...
public interface IFinancialRepository extends MongoRepository<Financial,String>/*, QueryDslPredicateExecutor<Financial>*/{
    ...
    Financial findByNameAndDateTime(String name, int year);

    @Query(value = "{ name : ?0 , company.name : ?0 }")
    Financial findByNameAndCompany(String name, String company);

    @Query(value = "{name:?0,financialDetails.year:?0,company.name:?0}")
    Financial findByNameAndDateCompany(String name,int year, String company);
